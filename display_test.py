...
def add_block(display, top_left_x, top_left_y, top_right_x, top_right_y, bottom_left_x, bottom_left_y, bottom_right_x, bottom_right_y, character, horizontal = True):
    if horizontal: 
        ...
        for i in range(ymin, ymax + 1):
            ...
            first_x_affected = top_left_x + ceil((bottom_left_x - top_left_x)/(bottom_left_y - top_left_y) * (i - top_left_y))
            last_x_affected = top_right_x + floor((bottom_right_x - top_right_x)/(bottom_right_y - top_right_y) * (i - top_right_y))
        ...
        for i in range(xmin, xmax + 1):
            # need to accommodate where the bounding edges are not the left or right edges - FIX
            first_y_affected = top_left_y + ceil((top_right_y - top_left_y)/(top_right_x - top_left_x) * (i - top_left_x))
            last_y_affected = bottom_left_y + floor((bottom_right_y - bottom_left_y)/(bottom_right_x - bottom_left_x) * (i - bottom_left_x))
            for j in range(first_y_affected, last_y_affected + 1):
                display[j] = replace_character(display[j], i, character)

display = ['.' * display_width] * display_height
add_block(display, 0, 0, unit_width * 3 + 3, 0, unit_width * 1 + 1, unit_height * 1 + 1, int((unit_width * 10 + 10)/3), unit_height * 1 + 1, 'O')
add_block(display, 0, 0, unit_width * 1 + 1, unit_height * 1 + 1, 0, unit_height * 3 + 3, unit_width * 1 + 1, int((unit_height * 10 + 10)/3), 'U', horizontal=False)
